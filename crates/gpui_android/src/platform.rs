...
impl AndroidPlatform {
    ...
    fn emit_app_lifecycle(&self, phase: AppLifecyclePhase) {
        // Take the callback before invoking it. GPUI callbacks can synchronously
        // reenter the platform and register a replacement callback.
    ...
    fn handle_main_event(&self, event: MainEvent<'_>) {
        match event {
            ...
            MainEvent::InsetsChanged { .. } => {
                log::info!("insets changed: content_rect={:?}", self.app.content_rect());
                if let Some(window) = self.window() {
                    window.update_insets(false);
                }
            }
            ...
            MainEvent::Start => self.emit_app_lifecycle(AppLifecyclePhase::Foreground),
            MainEvent::Resume { .. } => {
                if let Some(window) = self.window() {
                    window.set_active(true);
                }
                self.emit_app_lifecycle(AppLifecyclePhase::Active);
            }
            MainEvent::Pause => {
                if let Some(window) = self.window() {
                    window.set_active(false);
                    window.reset_soft_keyboard_request();
                }
                self.emit_app_lifecycle(AppLifecyclePhase::Inactive);
            }
            MainEvent::Stop => {
                if let Some(window) = self.window() {
                    window.set_active(false);
                ...
                self.emit_app_lifecycle(AppLifecyclePhase::Background);
            }
    ...
    fn maybe_request_frame(&self) {
        ...
        if let Some(window) = self.window() {
            ...
            events::tick_long_press(&window, &mut self.touch_gesture.borrow_mut());
            self.last_frame.set(Instant::now());
...
impl Platform for AndroidPlatform {
    ...
    fn register_url_scheme(&self, _url: &str) -> Task<Result<()>> {
        Task::ready(Err(anyhow::anyhow!(
            "register_url_scheme is not implemented on Android"
        )))
    }
