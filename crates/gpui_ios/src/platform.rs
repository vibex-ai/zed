use crate::{
    IosDisplay, IosWindow, dispatcher::IosDispatcher, id, nil, ns_string,
    window::string_from_ns_string,
};
use anyhow::{Result, anyhow};
use futures::channel::oneshot;
use gpui::{
    AnyWindowHandle, AppLifecyclePhase, BackgroundExecutor, ClipboardItem, CursorStyle,
...
thread_local! {
    ...
    static LIFECYCLE_CALLBACK: RefCell<Option<Box<dyn FnMut(AppLifecyclePhase)>>> = const { RefCell::new(None) };
    static MEMORY_WARNING_CALLBACK: RefCell<Option<Box<dyn FnMut()>>> = const { RefCell::new(None) };
}

fn emit_lifecycle(phase: AppLifecyclePhase) {
    let callback = LIFECYCLE_CALLBACK.with(|slot| slot.borrow_mut().take());
    if let Some(mut callback) = callback {
        callback(phase);
        LIFECYCLE_CALLBACK.with(|slot| {
...
extern "C" fn did_finish_launching(_this: &Object, _: Sel, _application: id, _options: id) -> BOOL {
    ...
    emit_lifecycle(AppLifecyclePhase::Active);
    YES
}

extern "C" fn will_resign_active(_this: &Object, _: Sel, _application: id) {
    emit_lifecycle(AppLifecyclePhase::Inactive);
}

extern "C" fn did_enter_background(_this: &Object, _: Sel, _application: id) {
    emit_lifecycle(AppLifecyclePhase::Background);
}

extern "C" fn will_enter_foreground(_this: &Object, _: Sel, _application: id) {
    emit_lifecycle(AppLifecyclePhase::Foreground);
}

extern "C" fn did_become_active(_this: &Object, _: Sel, _application: id) {
    emit_lifecycle(AppLifecyclePhase::Active);
}

extern "C" fn did_receive_memory_warning(_this: &Object, _: Sel, _application: id) {
    let callback = MEMORY_WARNING_CALLBACK.with(|slot| slot.borrow_mut().take());
    if let Some(mut callback) = callback {
        callback();
        MEMORY_WARNING_CALLBACK.with(|slot| {
            let mut slot = slot.borrow_mut();
            if slot.is_none() {
                *slot = Some(callback);
            }
...
impl IosPlatform {
    pub fn new() -> Self {
        Self::with_text_system(Arc::new(
            gpui_wgpu::CosmicTextSystem::new_without_system_fonts("Helvetica"),
        ))
    }

    /// Creates the platform with an application-provided text system.
    /// Mobile hosts that bundle fonts can use the default cosmic-text system
    /// and register those fonts through `PlatformTextSystem::add_fonts`.
...
impl Platform for IosPlatform {
    ...
    fn run(&self, on_finish_launching: Box<dyn 'static + FnOnce()>) {
        ...
        unsafe {
            ...
            UIApplicationMain(argc, argv.as_mut_ptr(), nil, delegate_class_name);
        }
    }
    ...
    fn open_window(
        ...
        let window = IosWindow::open(handle, self.gpu_context.clone())?;
    ...
    fn on_app_lifecycle(&self, callback: Box<dyn FnMut(AppLifecyclePhase)>) {
        LIFECYCLE_CALLBACK.set(Some(callback));
    }

    fn on_memory_warning(&self, callback: Box<dyn FnMut()>) {
        MEMORY_WARNING_CALLBACK.set(Some(callback));
    }
