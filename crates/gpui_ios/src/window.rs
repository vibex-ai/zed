use crate::{CGFloat, CGPoint, CGRect, CGSize, IosDisplay, UIEdgeInsets, id, nil, text_input};
...
use gpui::{
    ...
    MouseMoveEvent, MouseUpEvent, PinchEvent, Pixels, PlatformAtlas, PlatformDisplay,
    PlatformInput, PlatformInputHandler, PlatformWindow, Point, PromptButton, PromptLevel,
    RequestFrameOptions, Scene, ScrollDelta, ScrollWheelEvent, Size, TextInputStateChange,
    TouchPhase, WindowAppearance, WindowBackgroundAppearance, WindowBounds, WindowControlArea,
    WindowInsets, point, px, size,
};
...
/// A cloneable handle for the UIView-backed WGPU surface. The view is retained
/// by UIKit for the lifetime of the GPUI window, so wgpu can safely borrow the
/// raw pointer while recreating surfaces after lifecycle transitions.
#[derive(Clone, Copy, Debug)]
struct RawWindow {
    view: ptr::NonNull<c_void>,
}

// UIKit objects are main-thread objects. WGPU only uses these marker traits to
// move the handle into its surface creation closure; all calls remain on the
// main thread in this backend.
unsafe impl Send for RawWindow {}
unsafe impl Sync for RawWindow {}

impl RawWindow {
    fn new(view: id) -> anyhow::Result<Self> {
        let view = ptr::NonNull::new(view.cast()).ok_or_else(|| anyhow::anyhow!("nil UIView"))?;
        Ok(Self { view })
    }
}
...
impl rwh::HasWindowHandle for RawWindow {
    fn window_handle(&self) -> Result<rwh::WindowHandle<'_>, rwh::HandleError> {
        let handle = rwh::UiKitWindowHandle::new(self.view);
...
fn surface_config(size: Size<DevicePixels>) -> WgpuSurfaceConfig {
    WgpuSurfaceConfig {
        size,
        transparent: false,
        // Mailbox is preferable on mobile when available; WGPU falls back to
        // FIFO on devices whose CAMetalLayer does not expose it.
...
pub(crate) struct IosWindowState {
    ...
    insets_changed_callback: Option<Box<dyn FnMut(WindowInsets)>>,
...
impl IosWindow {
    ...
    pub(crate) fn open(
        ...
        unsafe {
            ...
            let raw_window = RawWindow::new(native_view)?;
            let renderer = WgpuRenderer::new(
                gpu_context.clone(),
                &raw_window,
                surface_config(bounds.size.to_device_pixels(scale_factor)),
                None,
            )?;
...
impl PlatformWindow for IosWindow {
    ...
    fn on_insets_changed(&self, callback: Box<dyn FnMut(WindowInsets)>) {
        self.0.borrow_mut().insets_changed_callback = Some(callback);
    }
    ...
    fn is_subpixel_rendering_supported(&self) -> bool {
        self.0.borrow().renderer.supports_dual_source_blending()
    }
    ...
    fn insets(&self) -> WindowInsets {
        self.0.borrow().insets.clone()
    }

    fn show_soft_keyboard(&self) {
        let text_input_view = self.0.borrow().text_input_view;
        unsafe {
...
fn gpui_view_class() -> &'static Class {
    ...
    REGISTER.call_once(|| {
        ...
        unsafe {
            // Give wgpu a CAMetalLayer-backed UIView. This keeps all Metal
            // ownership in wgpu while retaining UIKit's normal touch and
            // responder behavior.
...
fn apply_keyboard_overlap(window_state: &Rc<RefCell<IosWindowState>>, overlap: Pixels) {
    ...
    let callback = window_state.borrow_mut().insets_changed_callback.take();
    if let Some(mut callback) = callback {
        callback(insets);
        let mut state = window_state.borrow_mut();
        if state.insets_changed_callback.is_none() {
            state.insets_changed_callback = Some(callback);
        }
    }
}

/// Reads UIKit's safe-area geometry in points and publishes it through GPUI's
/// mobile inset contract. The keyboard is tracked separately by the keyboard
/// notifications above, so layout never changes the drawable size itself.
fn update_safe_area(window_state: &Rc<RefCell<IosWindowState>>) {
    let (native_view, keyboard_overlap, previous) = {
        let state = window_state.borrow();
        (
            state.native_view,
            state.keyboard_overlap,
            state.insets.clone(),
        )
    };
    let safe: UIEdgeInsets = unsafe { msg_send![native_view, safeAreaInsets] };
    let next = WindowInsets {
        safe_area: Edges {
            top: px(safe.top as f32),
            right: px(safe.right as f32),
            bottom: px(safe.bottom as f32),
            left: px(safe.left as f32),
        },
        ime: Edges {
            top: px(0.),
            right: px(0.),
            bottom: keyboard_overlap,
            left: px(0.),
        },
    };
    if next == previous {
    ...
    let callback = {
        ...
        state.insets_changed_callback.take()
    };
    if let Some(mut callback) = callback {
        callback(next);
        let mut state = window_state.borrow_mut();
...
fn restore_surface(window_state: &Rc<RefCell<IosWindowState>>) {
    let (raw_window, gpu_context, size, scale_factor) = {
        let state = window_state.borrow();
        (
            state.raw_window,
    ...
    let Some(instance) = gpu_context
        .borrow()
        .as_ref()
        .map(|context| context.instance.clone())
    else {
        log::warn!("cannot restore iOS WGPU surface before context initialization");
        return;
    };
    let result = window_state.borrow_mut().renderer.replace_surface(
        &raw_window,
        surface_config(size.to_device_pixels(scale_factor)),
        &instance,
    );
    match result {
        Ok(()) => window_state.borrow_mut().surface_configured = true,
        Err(error) => log::error!("failed to restore iOS WGPU surface: {error:#}"),
    }
}
